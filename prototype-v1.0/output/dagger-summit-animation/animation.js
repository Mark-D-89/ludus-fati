(() => {
  ...
  function drawAtmosphere(now) {
    ...
    for (const mote of motes) {
      ...
      context.beginPath();
      context.fillStyle = `rgba(247, 194, 139, ${mote.alpha * flicker})`;
    ...
    for (const glint of glints) {
      ...
      context.beginPath();
      context.strokeStyle = `rgba(235, 151, 93, ${(shimmer - .66) * .34})`;
      context.lineWidth = .7;
  ...
  function restart() {
    ...
    const animated = scene.querySelectorAll(
      ".world, .dagger-aura, .dagger-aura__slash, .atmosphere, .light-sweep, .birds, .quote-card, .journey__track i"
    );
    ...
    status.textContent = wasPaused
      ? "The journey returned to the Lower Bastions and remains paused."
      : "The journey restarted at the Lower Bastions.";
  }
  ...
  reduceMotion.addEventListener("change", () => {
    if (reduceMotion.matches) {
      status.textContent = "Reduced motion is enabled; the summit is shown as a still panorama.";
    } else {
      status.textContent = paused
        ? "The panoramic journey is paused."
        : "The panoramic journey is playing.";
