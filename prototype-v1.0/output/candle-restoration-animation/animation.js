(() => {
  ...
  function drawAtmosphere(now) {
    ...
    for (const mote of motes) {
      ...
      context.beginPath();
      context.fillStyle = `rgba(255, 222, 157, ${mote.alpha * flicker})`;
    ...
    for (const glint of glints) {
      ...
      context.beginPath();
      context.strokeStyle = `rgba(245, 193, 108, ${(shimmer - .66) * .34})`;
      context.lineWidth = .7;
  ...
  function restart() {
    ...
    const animated = scene.querySelectorAll(
      ".world, .beacon-aura, .beacon-aura__flame, .atmosphere, .light-sweep, .birds, .quote-card, .journey__track i"
    );
    ...
    status.textContent = wasPaused
      ? "The journey returned to the Ruined Quarter and remains paused."
      : "The journey restarted at the Ruined Quarter.";
  }
  ...
  reduceMotion.addEventListener("change", () => {
    if (reduceMotion.matches) {
      status.textContent = "Reduced motion is enabled; the restored world is shown as a still panorama.";
    } else {
      status.textContent = paused
        ? "The panoramic journey is paused."
        : "The panoramic journey is playing.";
