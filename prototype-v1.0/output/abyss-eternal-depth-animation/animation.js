(() => {
  ...
  function drawAtmosphere(now) {
    ...
    for (const mote of motes) {
      ...
      context.beginPath();
      context.fillStyle = `rgba(185, 195, 216, ${mote.alpha * flicker})`;
    ...
    for (const glint of glints) {
      ...
      context.beginPath();
      context.strokeStyle = `rgba(174, 189, 214, ${(shimmer - .66) * .34})`;
      context.lineWidth = .7;
  ...
  function restart() {
    ...
    const animated = scene.querySelectorAll(
      ".world, .eclipse-aura, .eclipse-aura__ring, .atmosphere, .light-sweep, .quote-card, .journey__track i"
    );
    ...
    status.textContent = wasPaused
      ? "The journey returned to the Last Observatory and remains paused."
      : "The journey restarted at the Last Observatory.";
  }
  ...
  reduceMotion.addEventListener("change", () => {
    if (reduceMotion.matches) {
      status.textContent = "Reduced motion is enabled; the Abyss is shown as a still panorama.";
    } else {
      status.textContent = paused
        ? "The panoramic journey is paused."
        : "The panoramic journey is playing.";
